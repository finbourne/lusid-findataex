    ...
    public class GetDataLicenseCallTest
    {
        ...
        public void Get_OnValidInstruments_ShouldReturnPrice()
        {
            ...
            Mock.Get(_perSecurityWs)
                .Setup(perWsMock => perWsMock.submitGetDataRequest(It.IsAny<submitGetDataRequestRequest>()))
                .Returns(submitGetDataRequestResponse) // return a submit request response that contains the response id used to poll for results
                .Callback<submitGetDataRequestRequest>(r => submitGetDataRequestRequest = r); //retrieve request argument to ensure properly constructed

            // Setup mock service to return successful data request. 
            retrieveGetDataResponseRequest retrieveGetDataResponseRequest = null;

            var retrieveGetDataResponseResponse = CreateRetrieveGetDataResponseResponse(responseId);
            Mock.Get(_perSecurityWs)
                .Setup(perWsMock => perWsMock.retrieveGetDataResponse(It.IsAny<retrieveGetDataResponseRequest>()))
                .Returns(retrieveGetDataResponseResponse) // return the expected data response from bbg
                .Callback<retrieveGetDataResponseRequest>(r => retrieveGetDataResponseRequest = r); //retrieve get data request argument that contains the response id. ensure it's properly constructed
